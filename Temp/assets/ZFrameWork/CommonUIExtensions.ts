...
export class CommonUIExtensions {
    ...
    public static pushContentToLayer<T extends ZCommonActivatableWidget>(layerName: string, widgetClass: ZClass<T>, callback: (widget)=>void): void {
        GameInstance.getInstance().getSubsystem(ZGameUIManagerSubsystem).currentPolicy.getRootLayout().pushWidgetToLayerStackAsync(layerName, widgetClass, callback)
    }
