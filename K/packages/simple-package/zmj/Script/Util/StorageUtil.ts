export class StorageUtil
{
    ...
    static getAllLocalData(gameData, callback: Function)
    {
        ...
        for (let key in gameData)
        {
            if(typeof gameData[key] === "object")
            {
                ...
                this.pureDataCache[key] = JSON.parse(JSON.stringify(gameData[key]));
            }
            else
            {
                this.pureDataCache[key] = gameData[key];
            ...
            for(let key in this.pureDataCache)
            {
                ...
                if(value && typeof value === "object")
                {
                    for(let childKey in value)
                    {
                        ...
                        this.pureDataCache[key][childKey] = JSON.parse(JSON.stringify(localValue));
    ...
    static rebindGameDataRef(key, value)
    {
        this.gameDataRef[key] = value;
    }
